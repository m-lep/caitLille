...
if st.session_state.current_index < TOTAL:
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # SECTION OFFRES DISPONIBLES - Affichée après la carte des résultats
    # ═══════════════════════════════════════════════════════════════════════
    st.markdown("---")
    st.markdown("### 🏠 Offres disponibles en location à Lille")
    ...
    with st.spinner("Chargement des offres..."):
        annonces_results = scraper_immosens(secteur="Lille", budget_min=budget_min, budget_max=budget_max, max_annonces=10)
    
    if not annonces_results:
        st.info("Aucune offre trouvée actuellement. Essayez de modifier votre budget.")
    else:
        # Styles CSS pour le carrousel (définis une seule fois)
        st.markdown(
            """
            <style>
                .carousel-container {
        ...
        cards_html = ""
        for annonce in annonces_results:
